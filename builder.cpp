...
public:
    ...
    void set_build_type(const std::string& type)
    {
        build_type_ = type;
    }
    ...
    int build()
    {
        ...
        // Common flags
        compile_flags += " -std=c++23 -Wall -Wextra -Wpedantic -Iinclude -Iinclude/core";
        
        if (output_type_ == "executable")
        {
            output_name = build_dir + "/" + "cpp-sdk";
        }
        else if (output_type_ == "static")
        {
            output_name = build_dir + "/lib" + "cpp-sdk" + ".a";
            compile_flags += " -c";
        }
        else if (output_type_ == "dynamic")
        {
            output_name = build_dir + "/lib" + "cpp-sdk" + ".so";
            compile_flags += " -fPIC";
            link_flags += " -shared";
        }
        
        std::cout << "Building cpp-sdk (" << build_type_ << ", " << output_type_ << ")..." << std::endl;
        ...
        else
        {
            ...
            if (output_type_ == "executable")
            {
                build_cmd += " -static";  // Static executable
            }
